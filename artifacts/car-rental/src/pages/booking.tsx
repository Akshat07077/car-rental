...
import { useGetCar, useCreateBooking, useCreatePaymentSession } from "@workspace/api-client-react";
import { Button } from "@/components/ui/button";
import { format, differenceInDays } from "date-fns";
import { ArrowLeft, Check, ShieldCheck, Car as CarIcon, CreditCard } from "lucide-react";
...
export default function Booking() {
  ...
  const { data: car, isLoading: isLoadingCar } = useGetCar(Number(carId));
  const createBooking = useCreateBooking();
  const createSession = useCreatePaymentSession();

  const [isProcessing, setIsProcessing] = useState(false);

  if (isLoadingAuth || isLoadingCar || !user) {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-card p-6 md:p-8 rounded-3xl border border-border shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                <div className="w-full sm:w-1/3 aspect-[4/3] rounded-xl overflow-hidden bg-muted relative">
                  {car.imageUrl ? (
                     <img src={car.imageUrl} alt={car.model} className="absolute inset-0 w-full h-full object-cover" />
                  ) : (
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-4 bg-muted/30 p-4 rounded-xl border border-border/50">
                    <div>
            ...
            <div className="bg-card p-6 md:p-8 rounded-3xl border border-border shadow-sm">
              <h2 className="text-xl font-bold font-display mb-6 flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-primary" /> Driver Details
              </h2>
          ...
          <div className="md:col-span-1">
            <div className="bg-card p-6 rounded-3xl border border-border shadow-xl shadow-black/5 sticky top-28">
              ...
              <div className="border-t border-border/50 pt-4 mb-8">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-3xl font-display text-primary">${total}</span>
                </div>
              </div>
              ...
              >
                {isProcessing ? (
                  <><Spinner className="mr-2 h-5 w-5" /> Processing...</>
                ) : (
