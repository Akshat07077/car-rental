...
import { useGetBooking, useGetPayment } from "@workspace/api-client-react";
...
export default function Confirmation() {
  ...
  const { data: booking, isLoading: isBookingLoading } = useGetBooking(Number(bookingId));
  const { data: payment, isLoading: isPaymentLoading } = useGetPayment(Number(bookingId));

  if (isBookingLoading || isPaymentLoading) {
    return <div className="min-h-screen flex justify-center items-center"><Spinner className="w-8 h-8 text-primary" /></div>;
  }
  ...
  return (
    <div className="bg-muted/20 min-h-screen py-20 px-4">
      <div className="max-w-2xl mx-auto text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        {isPaid ? (
          <div className="w-24 h-24 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-8 shadow-lg shadow-green-500/20">
        ...
        <h1 className="text-4xl md:text-5xl font-display font-extrabold tracking-tight mb-4 text-foreground">
          {isPaid ? "Payment Successful!" : "Booking Received!"}
        </h1>
        <p className="text-lg text-muted-foreground mb-12 max-w-lg mx-auto">
          {isPaid 
            ? "Your reservation is confirmed. We've sent a receipt and details to your email."
            : "Your booking is pending payment confirmation. You can check the status in your dashboard."}
        </p>

        <div className="bg-card text-left p-8 rounded-3xl border border-border shadow-xl shadow-black/5 mb-10">
          ...
          <div className="mt-8 pt-6 border-t border-border/50 flex justify-between items-center bg-muted/30 -mx-8 -mb-8 px-8 py-6 rounded-b-3xl">
            ...
            <span className="font-bold text-2xl font-display text-foreground">${booking.totalPrice}</span>
          </div>
        </div>
